...
public class LimeLightNT implements AutoCloseable {
  ...
  /**
   * Returns Vertical Offset From Crosshair To Target (LL1: -20.5 degrees to 20.5 degrees / LL2: -24.85 to 24.85
   * degrees)
   */
  ...
  /**
   * Capture pipeline latency (ms). Time between the end of the exposure of the middle row of the sensor to the
   * beginning of the tracking pipeline.
   */
  ...
  public double getTagCount() {
    // Robot transform in field-space. Translation (X,Y,Z) in meters Rotation(Roll,Pitch,Yaw) in degrees, total latency
    // (cl+tl), tag count, tag span, average tag distance from camera, average tag area (percentage of image)
  ...
  public LimelightPose getRobotPose() {
    var poseData = m_limelightTable.getEntry("botpose").getDoubleArray(new double[11]); // Translation (X,Y,Z)
  ...
  public LimelightPose getRobotPose(DriverStation.Alliance alliance) {
    var poseData =
        alliance == Alliance.Blue ? m_limelightTable.getEntry("botpose_wpiblue").getDoubleArray(new double[11])
  ...
  public LimelightPose getRobotPoseInTargetSpace() {
    var poseData = m_limelightTable.getEntry("botpose_targetspace").getDoubleArray(new double[11]); // Translation
                                                                                                    // (X,Y,Z)
                                                                                                    // Rotation(Roll,Pitch,Yaw)
  ...
  public LimelightPose getCameraPoseInTargetSpace() {
    var poseData = m_limelightTable.getEntry("camerapose_targetspace").getDoubleArray(new double[11]); // Translation
                                                                                                       // (X,Y,Z)
                                                                                                       // Rotation(Roll,Pitch,Yaw)
  ...
  public LimelightPose getCameraPoseInRobotSpace() {
    var poseData = m_limelightTable.getEntry("camerapose_robotspace").getDoubleArray(new double[11]); // Translation
                                                                                                      // (X,Y,Z)
                                                                                                      // Rotation(Roll,Pitch,Yaw)
  ...
  public LimelightPose getTargetPoseInCameraSpace() {
    var poseData = m_limelightTable.getEntry("targetpose_cameraspace").getDoubleArray(new double[11]); // Translation
                                                                                                       // (X,Y,Z)
                                                                                                       // Rotation(Roll,Pitch,Yaw)
  ...
  public LimelightPose getTargetPoseInRobotSpace() {
    var poseData = m_limelightTable.getEntry("targetpose_robotspace").getDoubleArray(new double[11]); // Translation
                                                                                                      // (X,Y,Z)
                                                                                                      // Rotation(Roll,Pitch,Yaw)
  ...
  /**
   * Sets limelight’s LED state. 0 = use the LED Mode set in the current pipeline. 1 = force off. 2 = force blink. 3 =
   * force on.
   * 
   * @param mode
   */
  ...
  /**
   * Sets limelight’s operation mode. 0 = Vision processor. 1 = Driver Camera (Increases exposure, disables vision
   * processing).
   * 
   * @param mode
   */
  ...
  public void setCameraPose(Pose3d pose) {
    var poseData = new double[] {
        ...
        Units.radiansToDegrees(pose.getRotation().getX()), Units.radiansToDegrees(pose.getRotation().getY()),
